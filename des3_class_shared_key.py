...
class DES3Class:
    ...
    def encrypt(self):
        ...
        # We derive a 24-byte from the user-provided key
    ...
    def decrypt(self):
        """Decrypt DES3 encrypted data to plain text"""
        # Use the same 3DES cipher to decrypt the encrypted message
...
def main():
    ...
    cd = DES3Class(plain_text, shared_key)

    cd.encrypt()
    cd.decrypt()

    # Print results
    print(f"  Original Text: {cd.plain_text}")
    print(f"     Shared Key: {cd.hashed_key_bytes.hex()}")
    print(f" Encrypted Data: {cd.encrypted_data.hex()}")
    print(f" Decrypted Text: {cd.decrypted_data} ")
