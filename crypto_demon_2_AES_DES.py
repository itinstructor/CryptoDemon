...
import tkinter as tk  # Tkinter for creating the graphical user interface
from cryptography.fernet import Fernet  # Fernet for key generation
from cryptography.hazmat.primitives import padding  # Padding for message length
...
class EncryptionApp:
    def __init__(self, root):
        ...
        self.encrypted_text_label.grid(
            row=2, column=0, columnspan=2)  # Grid layout position
        ...
        self.decrypted_text_label.grid(
            row=4, column=0, columnspan=2)  # Grid layout position

        self.generate_key_button = tk.Button(
            root, text="Generate Shared Key", command=self.generate_key)  # Button to generate a shared key
        self.generate_key_button.grid(
            row=3, column=0, columnspan=2)  # Grid layout position
        ...
        self.encryption_method_label = tk.Label(
            root, text="Encryption Method:")
        self.encryption_method_label.grid(
            row=6, column=0, columnspan=2)  # Grid layout position
        ...
        self.encryption_method_combo = tk.OptionMenu(
            root, self.encryption_method_var, "AES-256", "Triple DES", "ChaCha20Poly1305")  # Combo box for method selection
        self.encryption_method_combo.grid(
            row=7, column=0, columnspan=2)  # Grid layout position
    ...
    def encrypt_text(self):
        if self.key:
            text = self.plain_text_input.get()  # Get the plain text from the input field
            ...
            if encryption_method == "AES-256":
                ...
                cipher = Cipher(algorithms.ChaCha20(
                    self.key[:32], nonce=self.nonce), mode=None, backend=default_backend())
            ...
            encrypted_text = encryptor.update(
                padded_data) + encryptor.finalize()  # Encrypt the data

            self.encrypted_text_label.config(
                text=f"Encrypted Text: {encrypted_text.hex()}")  # Display the encrypted text

    def decrypt_text(self):
        if self.key:
            encrypted_text = bytes.fromhex(self.encrypted_text_label.cget(
                "text")[15:])  # Get the encrypted text from the label
            # Get the selected encryption method
            encryption_method = self.encryption_method_var.get()
            ...
            decrypted_data = decryptor.update(
                encrypted_text) + decryptor.finalize()  # Decrypt the data
            unpadder = padding.PKCS7(128).unpadder()  # Remove padding
            unpadded_data = unpadder.update(
                decrypted_data) + unpadder.finalize()  # Unpad the data
            decrypted_text = unpadded_data.decode()  # Convert the decrypted data to text

            self.decrypted_text_label.config(
                text=f"Decrypted Text: {decrypted_text}")  # Display the decrypted text
